...
function addMusique()
{
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                if (empty($_FILES["Cover"]["name"])) {
                        ...
                        $extension = substr($monimage, strrpos($moncover, "."));
                        ...
                        if (!(in_array($extension, $extensionsAutorisees_image))) {
                                ...
                                $chemincover = "/p2/projet2/sound/musique_" . $next . $extension;
                                rename($_FILES["Sound"]["tmp_name"], $repository . $chemincover);
                        }
                } else {
                        $chemincover = "";
                }
                ...
                try {
                        ...
                        $sql = "INSERT INTO Musique(Titre,Album,Artiste,Genre,Cover,Sound)
                                VALUES('" . addslashes($_POST['Titre']) . "','" . addslashes($_POST['Album']) . "','" . addslashes($_POST['Artiste']) . "','" . addslashes($_POST['Genre']) . "','" . addslashes($chemincover) . "','" . addslashes($cheminmusique) . "')";
                        $codb->exec($sql);
                        $codb = null;
